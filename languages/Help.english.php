...
$helptxt['attachment_image_compression'] = 'This sets the JPEF quality factor. A higher number increases image quality but also increase the file size. ';
...
$helptxt['attachment_image_reformat'] = 'Selecting this option will reformat any non-JPEG images into the JPEG format';

// Bad Behavior Start
$helptxt['enable_badbehavior'] = 'The badbehavior module examines HTTP requests of visits to your web site, and any suspicious requests are logged for later review.  The suspicious visit is shown an error page with instructions on how to view the site without triggering the bad behavior error message.';
$helptxt['badbehavior_verbose'] = 'Enables or disables verbose logging which includes all requests, not just failed ones.  This will log ALL activity to your forum, including all permitted valid activity.<br /><br /> Using verbose mode is not recommended as it can significantly slow down your site; it exists to capture data from live spammers which are not being blocked. You probably want to disable verbose, since it will create a HUGE db table in a short amount of time.';
$helptxt['badbehavior_logging'] = 'Normal HTTP request logging.<br /><br />Only blocked requests and a few suspicious (but permitted) requests are blocked and logged.<br /><br />Disabling this feature can improve site performance on high traffic sites but may result in additional spam.<br /><br />There is a "Scheduled Task" that Deletes Old Bad Behavior Log Entries greater than seven(7) days.  The Scheduled Task can be adjusted.  It is recommended that the old entries are deleted to protect the privacy of your members and Bad Behavior may uses the logs to detect spammers.';
$helptxt['badbehavior_strict'] = 'Strict Checking blocks more spam but may block some people.<br /><br />Strict: Bad Behavior operates in two blocking modes: normal and strict. In normal mode, some checks which could stop spam, but also block certain types of corporate and government users, are disabled.  When strict mode is enabled, Strict mode (intentionally) blocks a significant amount of non-RFC-compliant traffic, most of which is from corporate and various government agencies, such as Homeland Security, Justice, Interior, and etc.<br /><br />It is up to you whether you want to have the government reading your forum, or keep away the spammers.';
$helptxt['badbehavior_log_timeformat'] = 'Set the Bad Behavior Log time display in 12- or 24-hour format.';
$helptxt['badbehavior_offsite_forms'] = 'Allow form postings from other web sites.<br /><br />Bad Behavior normally prevents your site from receiving data posted from forms on other web sites. This prevents spammers from, e.g., using a Google cached version of your web site to send you spam. However, some web applications such as OpenID require that your site be able to receive form data in this way.<br /><br />If you are running OpenID, enable this option.';
$helptxt['badbehavior_eu_cookie'] = 'Select this option if you believe Bad Behavior\'s site security cookie is not exempt from the 2012 EU cookie regulation. <a href="http://bad-behavior.ioerror.us/2012/05/03/bad-behavior-2-2-4/">More info</a>';
$helptxt['badbehavior_reverse_proxy'] = 'If you are using Bad Behavior behind a reverse proxy, load balancer, HTTP accelerator, content cache or similar technology, enable the Reverse Proxy option.<br /><br />If you have a chain of two or more reverse proxies between your server and the public Internet, you must specify <em>all</em> of the IP address ranges (in CIDR format) of all of your proxy servers, load balancers, etc. Otherwise, Bad Behavior may be unable to determine the clients true IP address.<br /><br />In addition, your reverse proxy servers must set the IP address of the Internet client from which they received the request in an HTTP header. If you don\'t specify a header, <a href="http://en.wikipedia.org/wiki/X-Forwarded-For">X-Forwarded-For</a> will be used. Most proxy servers already support X-Forwarded-For and you would then only need to ensure that it is enabled on your proxy servers. Some other header names in common use include <u>X-Real-Ip</u> (nginx) and <u>Cf-Connecting-Ip</u> (CloudFlare).<br /><br />X-Forwarded-For is the default.';
$helptxt['badbehavior_reverse_proxy_header'] = 'Header containing Internet clients IP address. When Reverse Proxy is enabled, Bad Behavior checks this header to locate the true IP address of the connecting client.';
$helptxt['badbehavior_reverse_proxy_addresses'] = 'IP address or CIDR format address ranges for your proxy servers (one per line).<br /><br />An array of IP address or CIDR netblocks which Bad Behavior trusts to provide reliable information in the HTTP header given above. If no addresses are given, Bad Behavior will assume that the HTTP header given is always trustworthy and that the right-most IP address appearing in the header is correct. If you have a chain of two or more proxies this is probably not what you want; in this scenario you should either set this option and provide all proxy server IP addresses (or ranges) which could conceivably handle the request, or have your edge servers set a unique HTTP header with the client\s IP address. For instance, when using CloudFlare, it is impossible to provide a list of IP addresses, so you would set the HTTP header to CloudFlare\'s provided CF-Connecting-IP header instead.';
$helptxt['badbehavior_email_allow'] = 'When this option is enabled the administrator will be sent an email every time a spammer is blocked<br /><br />Any site thats been online more than a month is going to send out dozens, hundreds or even thousands of such notifications DAILY.<br /><br />This option is for start-up testing ONLY.';
$helptxt['badbehavior_display_stats'] = 'Show a notification in the footer of how many blocks Bad Behavior has made.';
$helptxt['badbehavior_roundtripdns'] = 'Perform a Round trip DNS verification for the major Search Engines like MSN, Google and Yahoo.<br /><br />Cache is recomended for this option.<br /><br /><b>WARNING:</b> This option may not work with Ubuntu 10.04 Servers.';
$helptxt['badbehavior_cache_duration'] = 'The buffer time in seconds that the Bad Behavior cache is allowed to keep information for a readable IP address between Bad Behavior look-ups.  Because bots can look at multiple pages in your forum in a short amount of time this setting will allow either them or a visitor to view the forum or see the banned message within the duration using the least amount of processing effort.<br /><br />The time delay can be set between 0 and 99 seconds.  Zero disables this feature.  This number should be adjusted as low as possible based on server load.  Default is 0 seconds.<br /><br />There is a "Scheduled Task" that Purges all Bad Behavior cache files older than one day.  If desired this task can be adjusted.';
$helptxt['badbehavior_httpbl_key'] ='Access Keys are 12-alpha characters (no numbers). They are lower-case. You should copy the Access Key exactly as it appears at the <a href="http://www.projecthoneypot.org/"> Project Honey Pot network</a>.';
$helptxt['badbehavior_httpbl_threat'] ='Threat Scores are a rough guide to determine the threat a particular IP address may pose to your site. Threat Scores should be treated as a rough measure. Threat Scores range from 0-255, however they follow a logrithmic scale which makes it extremely unlikely that a threat score over 200 will ever be seen.';
$helptxt['badbehavior_httpbl_maxage'] ='Represents the number of days since last activity. This value ranges from 0 days to 255 days. This value is useful in helping you assess how "stale" the information provided by http:BL is and therefore the extent to which you should rely on it.';
$helptxt['badbehavior_httpbl_word'] ='A random interesting word which will be used in the Honeypot link on the warning page when someone is blocked.';
$helptxt['badbehavior_httpbl_link'] ='Your Honeypot (spam trap) link that will be placed on the Warning page. The link is used to catch bots and place them in the Project Honey Pot network.  The link can be one of your own <a href="http://www.projecthoneypot.org/manage_honey_pots.php">Honey Pots</a> or a <a href="http://www.projecthoneypot.org/manage_quicklink.php">QuickLink</a>.';
$helptxt['badbehavior_block_ua'] = 'The visitors with no defined user agent are normally automatically permitted.  Check this option to block them.<br /><br /><b>WARNING:</b> This option may block Paypal.';
$helptxt['badbehavior_limitfirst'] = 'Limits the amount of honeypots on the first page to to not alter the looks of the forum.';
$helptxt['badbehavior_ip'] = 'IP address or CIDR format address ranges to be whitelisted (one per line)<br /><br /><b>WARNING:</b> Inappropriate whitelisting WILL expose you to spam, or cause Bad Behavior to stop functioning entirely! DO NOT WHITELIST unless you are 100% CERTAIN that you should.';
$helptxt['badbehavior_url'] = 'URL fragments beginning with the / after your web site hostname (one per line).<br /><br />URLs are matched from the first / after the server name up to, but not including, the ? (if any). The URL to be whitelisted is a URL on YOUR site.  A partial URL match is permitted, so URL whitelist entries should be as specific as possible, but no more specific than necessary. For instance, "/example" would match "/example.php" and "/example/address".<br /><br /><b>WARNING:</b> Inappropriate whitelisting WILL expose you to spam, or cause Bad Behavior to stop functioning entirely! DO NOT WHITELIST unless you are 100% CERTAIN that you should.';
$helptxt['badbehavior_useragent'] = 'User agent strings to be whitelisted.  Exact match only. (one per line)<br /><br />Example "Mozilla/4.0 (It\'s me, let me in)"<br /><br /><b>WARNING:</b> Inappropriate whitelisting WILL expose you to spam, or cause Bad Behavior to stop functioning entirely! DO NOT WHITELIST unless you are 100% CERTAIN that you should.';
// Bad Behavior End

$helptxt['vigLinkCode'] = 'Copy your API key and paste it here.<br />Need a VigLink API Key? <a href="http://www.viglink.com/?vgref=8799">Click Here</a>';
